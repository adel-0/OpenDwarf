...
- **The `A_ATTACK` menu** (`df.global.game.main_interface.adventure.attack`, focus `dungeonmode/Attack` — an UNKNOWN focus, so opening it routes to the L3 escape hatch): `mode` (0 = pick target), `unit_choice` (vector of `df.unit` targets under the cursor), `combat_list` (fields `target/strike/wrestle/a_item_list/a_bp_list/...` — attacker weapon × target body-part, populated only after a target is chosen), `selected_bp`, `selected_item_id`, `allow_strike`/`allow_wrestle`. Keyboard `SELECT` does NOT advance `mode 0` (cursor/mouse-driven, like the conversation choice list); the right panel does not render to `extract_screen_text`. Navigating this reliably is the unbuilt `CombatStrike` work — do NOT ship blind.
