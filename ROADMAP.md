...
| One-shot conversations; no per-NPC topic memory | PARTIAL — **submenu/identity-trap handling now FIXED (2026-06-12)**: reliable phase-2 choice selection (`scroll=idx*3` + deferred text-matched pixel-click, LIVE-VERIFIED) + identity-trap filtering in `registry.py`. The agent can now navigate submenus (incl. the 98-item "Ask for directions" list) without clicking the wrong row or tripping the identity screen. **Still open**: `ConverseSkill` multi-turn re-engagement + per-NPC asked-topics dedup (3.1 tail) — unblocked by the selection fix, next build. See `MEMORY.md` → conversation-flail-bottleneck. |
...
3.1 **Multi-turn conversations.** (PARTIAL: `TalkToSkill` exists — route + initiate. **Reliable choice selection + identity-trap filtering DONE 2026-06-12** — see "Last major change". Remaining: multi-turn re-engagement + topic dedup.) DF ends dialogue after single exchanges. Add a `ConverseSkill` that re-initiates `talk` with the *same* NPC (resolved by `hist_fig_id`) for follow-ups, with an asked-topics set per NPC persisted as a semantic memory note, so the agent works through rumor → details → directions without re-asking. The dedup data also feeds the prompt ("you already asked X about the bandit camp").
