...
| One-shot conversations; no per-NPC topic memory | PARTIAL — **submenu/identity-trap handling now FIXED (2026-06-12)**: reliable phase-2 choice selection (`scroll=idx*3` + deferred text-matched pixel-click, LIVE-VERIFIED) + identity-trap filtering in `registry.py`. The agent can now navigate submenus (incl. the 98-item "Ask for directions" list) without clicking the wrong row or tripping the identity screen. **Per-NPC asked-topics dedup DONE 2026-06-12** (`AskedTopics`: persisted normalized topic sets, inline `[already asked]` choice annotations + prompt hint, complements the `_ConversationGuard` streak counter). **Still open**: folding auto-re-initiate-talk into a single `ConverseSkill` — the LLM still re-engages turn-by-turn via `talk_to`, now dedup-informed. See `MEMORY.md` → conversation-flail-bottleneck. |
...
3.1 **Multi-turn conversations.** (PARTIAL: `TalkToSkill` exists — route + initiate. **Reliable choice selection + identity-trap filtering DONE 2026-06-12**. **Per-NPC asked-topics dedup DONE 2026-06-12** — `AskedTopics` (`memory/asked_topics.py`, persisted `memory/asked_topics.json`) records each phase-2 dialogue choice against the NPC's `hist_fig_id` (normalized so cross-prefix variants dedupe; nav/meta choices excluded); already-asked choices are marked `[already asked]` inline + a `_build_hint` note ("with X you have already asked about: …") so re-engagement pursues *new* topics. Remaining: fold automatic re-initiation of `talk` with the *same* NPC into a single multi-step `ConverseSkill` so the agent works through rumor → details → directions in one skill rather than the LLM re-issuing `talk_to` each round. The dedup data already feeds the prompt; the skill would just remove the per-round LLM hop.) DF ends dialogue after single exchanges.
