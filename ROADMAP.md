...
**Currently open (as of 2026-06-17):**
- **Validated-gameplay playtests (2026-06-16/17)** — three recorded long-run `/playtest` sessions (the missing "let reality re-prioritize" loop) found and fixed **two total-blocker wedges**, each invisible to existing detectors, then confirmed the fixes live. (1) **`status.popups` input wedge — FIXED** (commit `dabcf51`): DF's centered "mega" world/agreement popups ("You are now in control of X", fired by a lord adventurer) render over `dungeonmode/Default` with a "More" pager, are neither a viewscreen nor a `main_interface` widget, and swallow ALL input; `clickok` (Okay-only scan) never caught them. An entire 11-min run did nothing (tick + position frozen the whole time). Fix: per-tick `_drain_popups()` auto-handler (`opendwarf--drainpopups.lua` erases the vector — clicks/keys don't advance it; LIVE-VERIFIED) + `GameState.popup_messages`; held over a full hour with no re-wedge. (2) **`read_screen`/`escape` perception loop at Default — FIXED** (commit `034e9aa`): with the popup wedge cleared, a 60-min run spun ~21 turns reading the always-on compass HUD (N/NE/SSW…) as a "direction menu" and churning `read_screen → press:ESCAPE → escape` against an actionable screen (26 `read_screen`, `console_error: Invalid keycode ESCAPE`), never advancing. Fix: alias hallucinated `press:<key>` names to real keycodes (`ESCAPE`→`LEAVESCREEN`), gate `read_screen` off Default (HUD text misleads the LLM), and a **frozen-progress circuit breaker** (`_FROZEN_TURN_LIMIT` turns with no tick + no movement on a normal screen ⇒ ban the thrash actions + corrective hint; conversation/travel excluded). 15-min post-fix run: `read_screen` 26→0, `escape` 6→0, `press` 7→1, `console_error` 1→0 — agent explores/talks/pathfinds continuously. **Binding constraint now (refuted "combat never fires" is moot until this clears): conversation/quest-log churn that never converts into leaving town** — the agent re-reads the quest log and re-converses the same NPCs but does not commit to a `journey`; next concrete build is **commit-to-travel** (force a `journey`/`explore` once `conv_guard` marks an NPC exhausted; stand up `JourneyBehavior` autopilot so travel isn't re-litigated per turn). Regression tapes: `logs/session_20260616_*` / `logs/session_20260617_173014/tape.jsonl`.
- **Full-journey LIVE-VERIFY** — an observed unsupervised `JourneyBehavior` trek to a distant site; ChunkMap absolute-coord stability across the region change + topo-snap (3.4 remainder). **ARRIVAL ACHIEVED 2026-06-14** (commit pending): a no-LLM scratch-harness trek (loop's exact behavior cadence) drove the adventurer from LEAP TEMPLE all the way into **TOME MOUTH** (17 embark-tiles S; ended `site_name=TOME MOUTH`, `fast_travel_active=False`, dist 1) — the first end-to-end autonomous arrival at a distant site. This closes the prior "agent did NOT arrive / detour router wanders" gap. **Root cause + fix**: the detour router only rotated headings on *positional stall* (army pinned), so a heading that kept the army moving but *away* from the goal never triggered a rotation — the army walked unproductive headings 35+ steps (live-observed: drift 14→20 embark tiles, stuck). Fix: added a second heading-failure signal in `JourneyBehavior._steer` — **no-progress** (army moves `_NO_PROGRESS_LIMIT`=8 steers without ever beating its best distance ⇒ rotate the detour heading), keyed on "no new best" rather than strict per-step increase so a *staircase* drift (distance plateaus between increases) still accumulates. Post-fix live trek closed 18→7→arrival, with the no-progress counter firing on drifts and the rotated headings recovering each time. **ChunkMap region-change coord stability — VERIFIED 2026-06-14**: an instrumented scratch trek confirmed the absolute-coordinate machinery holds across a region-origin change — the local→absolute offset recomputed cleanly to a pure region multiple (`%16==0`), **all prior origin chunks were preserved (zero overwritten)**, new chunks landed at globally-consistent absolute coords, and the adventurer's ABS delta matched the on-foot move. (Absolute = `region_x*16 + local` is a global frame, so chunks from different sites occupy disjoint regions and never collide.) **Full LLM loop in the seat — EXERCISED 2026-06-14** (deepseek-flash, real interrupt checker): the LLM issued `journey:207`, recovered from the designed obstructed-start hand-backs by walking to open ground, **formed the travel army and completed a real leg (15→8 embark-tiles N)** — then oscillated at a northern terrain barrier. Two findings: (1) the LLM gravitates to one-shot `goto_site` (no detour routing) over `journey` (the routing autopilot), so it kept re-hitting the same barrier instead of engaging the detour router; (2) a **goal-completion bug, now FIXED**: `REACH_SITE` fired on *any* non-empty `site_name` — completing "fast-travel to LEAP TEMPLE" at the origin TOME MOUTH and derailing the plan to "talk to an NPC in LEAP TEMPLE" 15 tiles early. Fix: REACH_SITE now requires a change away from the step's origin site, and `REACH_SITE`/`TRAVEL` steps get a 30-turn (not 6-turn) fallback so a slow journey isn't force-advanced (`goals/model.py` + `manager.py`, `tests/test_plan_steps.py`). **Still open**: a clean *full arrival* under the LLM loop remains gated on routing quality (the bug-router can't pass terrain barriers without a world terrain map) + the LLM's `goto_site`-over-`journey` preference; and topo-snap (3.4, trap #5) still depends on the unbuilt Layer-2 topo graph.
...
- **Eval harness** (Phase 6.3 / NORTHSTAR M4): runner (`evals/run.py`: live / `--judge-only`), 4 scenario YAMLs, predicate language, and the **escape-hatch review doc** (`evals/review.py` → `logs/REVIEW.md`, the L3→L1 promotion queue clustering `escape_hatch`/`console_error`/`unstick_failed` by focus/action/outcome) are all built + unit-tested; `review.py` LIVE-RUN against real logs surfaced (and we fixed) a real latent bug — the `press:` escape hatch sent natural-language key aliases (`ESCAPE`, `ENTER`, …) that are not valid `df.interface_key` names and silently no-op'd. The in-memory simulator + `--offline` eval path were since removed (tautological/false-assurance); the kept record/replay tape (`opendwarf/sim/record.py`, the playtest skill's capture half) is the replay-into-loop harness that replaces it. **Still open**: capturing the live DF scenario save-states (`wolf_encounter`/`patrol_town`/`grind_wilderness`/`town_shop_adjacent`) — a manual DF step — so live scenarios can actually run.
...
> **Update (2026-06-17):** the validated-gameplay `/playtest` runs (see "Currently open" top of file) revisited this. The `read_screen`/`press`/`escape` churn was traced to two concrete wedges (the `status.popups` input wedge and the Default-screen perception loop) — **both now FIXED** (`dabcf51`, `034e9aa`); post-fix runs show no thrash. The **conversation-flail finding below still holds** as the (now non-blocking) #1 time-sink, refined to: the agent talks but never *commits to leaving town* — see the commit-to-travel next-build note up top.
...
7.3 **Process supervision**: session supervisor in `main.py` (retry with backoff, wait out a dead DF), stall watchdog (N no-change ticks with no active skill → escalate). The RPC client already reconnects on timeout. **PARTIAL (2026-06-17)**: the tactical-loop half of the stall watchdog landed as the **frozen-progress circuit breaker** (`_FROZEN_TURN_LIMIT` turns with no tick advance AND no movement on a normal/known screen ⇒ ban thrash actions + corrective hint; conversation/travel excluded) — `034e9aa`. Still open: the `main.py` session supervisor (retry/backoff, wait out a dead DF).
...
- **Centered "mega" popups (LIVE-VERIFIED 2026-06-16)**: `df.global.world.status.popups` (vector of `popup_message`, schema `mega_announcementst`) are full-screen world/agreement notices ("You are now in control of X") shown with a "More" pager. They draw OVER `dungeonmode/Default` — focus stays Default, `TAKING_INPUT`, every `main_interface` widget `open=false`, no viewscreen — yet they swallow ALL input (movement, fast travel, conversation) and a lord adventurer's start cascades many (regenerated on movement into controlled territory). `simulateInput` mouse-clicks (any coord/precise/press-release) and keys do NOT advance them; **erasing the vector entries does** (erase-only — DF owns/frees the `popup_message` objects, so `:delete()` would double-free). Drained per-tick by `opendwarf--drainpopups.lua` + the loop's `_drain_popups()` auto-handler. Distinct from the Okay-button modal (clickok) — different button, different struct.
