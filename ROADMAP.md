...
- **Full-journey LIVE-VERIFY** — an observed unsupervised `JourneyBehavior` trek to a distant site; ChunkMap absolute-coord stability across the region change + topo-snap (3.4 remainder). **Progress 2026-06-14**: a no-LLM scratch-harness trek toward TOME MOUTH (17 embark-tiles S, loop's exact behavior cadence) surfaced + fixed a hard blocker — the `StallWatchdog` keyed positional progress on `adventurer_position`, which is None throughout fast travel, so a correctly-steering journey false-stalled at 20 steps (fix: fold `fast_travel_army_pos` into the fingerprint; commit `9318851`). Post-fix the journey ran 104 steps, army formed on the first move, distance closed 17→6, ChunkMap abs coords stayed stable across the trek. **Still open**: the agent did NOT *arrive* — the collision-feedback detour router can't close the final gap to a town center and wanders (moving the army but away from goal counts as non-stall, so it walks unproductive headings); this is the "multi-leg routing *quality*" gap. Also still unexercised: a clean single-region-boundary crossing onto a *new* site + topo-snap (the trek oscillated near the origin). The detour router should treat sustained distance-regression as a heading-failure signal, not only positional stall.
