...
7.1 **Death handling**: detect death (LIVE-VERIFY focus strings), wire `PostmortemBuffer.generate_and_append` (exists, unwired), flush memory/reflection, archive per-life artifacts. (If the learn-across-lives loop proves valuable sooner, the postmortem wiring alone is cheap and can be pulled forward.)
