...
- **Full combat grind LIVE-VERIFY** — `GrindCombatBehavior` SEEK+A* is live-verified; a real hostile encounter (attack/flee/death) has **never** run end-to-end. Blocked on getting the agent out of town (conversation) or to a lair (journey).
- **Attack depth** (2.1 / NORTHSTAR M2): `A_ATTACK` is still blind — no `CombatStrike` target/strike selection.
