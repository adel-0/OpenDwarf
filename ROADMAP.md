...
- **Full combat grind LIVE-VERIFY** — PARTIAL (2026-06-13): exercised live against 6 wild wolves. Detection + targeting + the engage loop now run end-to-end (`GrindCombatBehavior` detects huntable wildlife, closes distance, and bump-strikes *hostiles*). **Root cause found + fixed**: wild creatures are never `isDanger`, even adjacent — they never entered `hostile_units`, so every combat path was unreachable; new `GameState.huntable_units` fixes detection (see CLAUDE.md → Combat targeting). **Remaining gap**: a clean KILL never completed — these wolves *flee* the adventurer, and (decisively) **bump-to-attack does not strike neutral wildlife** (it opens the `dungeonmode/Attack` menu). So killing wildlife is blocked on **attack depth** below, not on detection. Death/flee in a real hostile encounter still unexercised.
- **Attack depth** (2.1 / NORTHSTAR M2): `A_ATTACK` opens `dungeonmode/Attack` — an unmodeled cursor/target menu (`unit_choice` → `combat_list` body-part × weapon → confirm; `SELECT` doesn't advance it). Now the **critical** combat blocker: neutral wildlife can only be attacked through this menu (bump no-ops on them — LIVE-VERIFIED). Until `CombatStrike` automates it, opening the menu routes to the L3 escape hatch (it's an unknown focus). Menu structure documented in CLAUDE.md.
...
- **Eval harness** (Phase 6.3 / NORTHSTAR M4): scenario save-states not yet built — the flywheel's verification gate.
