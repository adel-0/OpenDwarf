...
- **Full combat grind LIVE-VERIFY** — combat now runs end-to-end: detection (`huntable_units`), targeting, SEEK/approach, and the ENGAGE strike (neutral wildlife via the attack menu, hostiles via bump) are all LIVE-VERIFIED against wild wolves. **Remaining gap**: a wolf has not yet been killed to death (these wolves *flee* the strong adventurer and the grind hasn't been run to a kill/level-up tally), and a real *hostile* encounter (danger/flee/`in_combat` semantics, the M2 ≥3-level overnight exit criterion) is still unexercised — gated on the wolf-survival eval scenario + reaching a hostile.
